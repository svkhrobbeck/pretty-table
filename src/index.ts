...
class PrettyTable {
  ...
  private getColumns(): string[] {
    return Array.from(new Set(this.table.reduce<string[]>((acc, curr) => acc.concat(Object.keys(curr)), [])));
  }
  ...
  private getPadConfig(): { [key: string]: number } {
    ...
    return columns.reduce(
      (acc, col) => {
        const maxColLength = Math.max(col.length, ...this.table.map(row => (row[col] ? String(row[col]).length : 0)));
  ...
  private getBody(): string {
    const padConfig = this.getPadConfig();
    const columns = this.getColumns();
    return this.table.map(row => `║${columns.map(col => String(row[col] ?? "-").padEnd(padConfig[col])).join("║")}`).join("║\n") + "║";
  }
